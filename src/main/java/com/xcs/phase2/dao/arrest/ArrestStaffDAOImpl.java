...
@Service
@Transactional
public class ArrestStaffDAOImpl extends ArrestExt implements ArrestStaffDAO{

	private static final Logger log = LoggerFactory.getLogger(ArrestStaffDAOImpl.class);

	@Override
	@Transactional(rollbackFor = Exception.class)
	public ArrestStaffinsAllResponse ArrestStaffinsAll(List<ArrestStaff> req) {
		
		ArrestStaffinsAllResponse res = new ArrestStaffinsAllResponse();
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public Boolean ArrestStaffupdByCon(List<ArrestStaff> request) {
		...
		if(request != null) {
		    ...
		    if(request.size() > 0){
		    	...
		    	for(ArrestStaff req : request){
		    	
		    		StringBuilder sqlBuilderSub = new StringBuilder()
		    				.append("UPDATE OPS_ARREST_STAFF "
		    						+ "SET "
	...
	public Boolean ArrestStaffupdDelete(List<ArrestStaffupdDeleteReq> req) {
		...
		if(req != null) {
		    ...
		    if(req.size() > 0){
		    	...
		    	for(ArrestStaffupdDeleteReq item : req){
		    		
		    		StringBuilder sqlBuilder = new StringBuilder().append("UPDATE OPS_ARREST_STAFF SET IS_ACTIVE = '0' WHERE STAFF_ID = '"+item.getSTAFF_ID()+"' ");

...
import com.xcs.phase2.response.lawsult.LawsuitStaffResponse;
...
@Service
@Transactional
public class LawsuitStaffDAOImpl extends LawsultExt implements LawsuitStaffDAO{

	private static final Logger log = LoggerFactory.getLogger(LawsuitStaffDAOImpl.class);

	@Override
	public LawsuitStaffinsAllResponse LawsuitStaffinsAll(List<LawsuitStaff> req) {
		
		LawsuitStaffinsAllResponse res = new LawsuitStaffinsAllResponse();
	...
	@Override
	public Boolean LawsuitStaffUpdate(List<LawsuitStaff> request) {
	...
	public Boolean LawsuitStaffupdDelete(List<LawsuitStaffupdDeleteReq> req) {
		...
		if(req != null) {
		    ...
		    if(req.size() > 0){
		    	...
		    	for(LawsuitStaffupdDeleteReq item : req){
		    		
		    		StringBuilder sqlBuilder = new StringBuilder().append("UPDATE OPS_LAWSUIT_STAFF SET IS_ACTIVE = '0' WHERE STAFF_ID = '"+item.getSTAFF_ID()+"' ");
